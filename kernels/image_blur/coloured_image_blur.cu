...
// Image blur kernel
__global__ void colouredImageBlurKernel(unsigned char *Image, unsigned char *Res, int width, int height, int channel) {
...
void imageBlurDevice(unsigned char *Image_h, unsigned char *Res_h, int width, int height, int channels) {
    ...
    colouredImageBlurKernel<<<grid_dim, block_dim>>>(Image_d, Res_d, width, height, channels);
...
int main () {
    ...
    *Res = stbi_write_png("coloured_image_blur_output.png", width, height, output_channels, Res, width * output_channels);
