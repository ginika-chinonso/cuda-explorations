...
__global__ void basic3DConvolutionKernel(float *In, float *Out, float *Filter) {
    ...
    for (int fRow = 0 ; fRow < FILTER_DIM; ++fRow) {
      for (int fCol = 0; fCol < FILTER_DIM; ++fCol) {
        ...
        if (in_x >= 0 && in_x < MATRIX_DIM && in_y >= 0 && in_y < MATRIX_DIM && in_z >= 0 && in_z < MATRIX_DIM) {
          res += Filter[(fDepth * FILTER_DIM * FILTER_DIM) + (fRow * FILTER_DIM) + fCol] * In[(in_z * MATRIX_DIM * MATRIX_DIM) + (in_y * MATRIX_DIM) + in_x];
        } 
...
void basic3DConvolutionFunctionDevice(float *In, float *Out, float *Filter) {
  
  // Declare variables for the Input, Output and Filter matrices
  float *In_d, *Out_d, *Filter_d;

  // Allocate memory for the Input, Output and Filter matrices on the device
  ...
  cudaMalloc(&Filter_d, FILTER_DIM * FILTER_DIM * FILTER_DIM * sizeof(float));

  // Copy input and filter matrix from the host to device
  cudaMemcpy(In_d, In, MATRIX_DIM * MATRIX_DIM * MATRIX_DIM * sizeof(float), cudaMemcpyHostToDevice);
  cudaMemcpy(Filter_d, Filter, FILTER_DIM * FILTER_DIM * FILTER_DIM * sizeof(float), cudaMemcpyHostToDevice);
  ...
  // call the  kernel
  basic3DConvolutionKernel<<<grid_dim, block_dim>>>(In_d, Out_d, Filter_d);
...
int main() {
  ...
  // Call device fuction
  basic3DConvolutionFunctionDevice(In, Out, Filter);
